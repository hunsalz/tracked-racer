...
<custom-style>
  ...
  <style is="custom-style">
...
</custom-style>`;template$5.setAttribute("style","display: none;");document.head.appendChild(template$5.content);class TrackedRacerApp extends PolymerElement{static get template(){return html`
      <style>
        :host {
          display: block;
          @apply --paper-font-common-base;
    ...
    `}static get properties(){return{url:{type:String,notify:!0,computed:"__computeWSUrl()"},state:{type:Number,observer:"__handleWSStateChanges",notify:!0},range:{type:Number,value:25},step:{type:Number,value:5},motorA:{type:Number,value:0,notify:!0,observer:"__changed"},motorB:{type:Number,value:0,notify:!0,observer:"__changed"}}}__changed(){this.$.ws.send({motorA:this.motorA,motorB:this.motorB,mode:"absolute"},!0)}__handleWSStateChanges(){var _color="var(--paper-orange-700)";switch(this.state){case 1:_color="var(--paper-green-700)";break;case 3:_color="var(--paper-red-700)";break;default:_color="var(--paper-orange-700)";}this.updateStyles({"--state-color":_color})}__getPercent(value){return 100/this.range*value}__computeWSUrl(){console.log(window.location.hostname);console.log("hostname.endsWith(github.io)",window.location.hostname.endsWith("github.io"));let hostname=window.location.hostname;if("127.0.0.1"===hostname||hostname.endsWith("github.io")){let wsUrlDev="wss://echo.websocket.org/";console.warn(hostname+" is defined as dev environment. Use "+wsUrlDev+" as mock service.");return wsUrlDev}else{return"wss://echo.websocket.org/"}}}window.customElements.define("tracked-racer-app",TrackedRacerApp)});